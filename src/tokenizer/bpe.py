...
def train_bpe(input_path, vocab_size, special_tokens) \
    -> tuple[dict[int, bytes], list[dict[bytes, bytes]]]:
    # 初始化词表
    ...
    with open(input_path, 'r', encoding='utf8') as f:
        text = f.read()
    if special_tokens:
        special_regix = '|'.join(re.escape(t) for t in special_tokens)
        corpus = re.split(special_regix, text)
    ...
    word_list = []
    word_freq = []
    for word, freq in raw_counter.items():
...
def save_tokenizer_file(vocab, merges, out_dir):
    """
    保存词表到指定目录
    """
    os.makedirs(out_dir, exist_ok=True)
    bytes_encoder = byte_to_unicode()
    json_vocab = {
        k:"".join([bytes_encoder[b] for b in v]) for k, v in vocab.items()
    }
    with open(os.path.join(out_dir, 'vocab.json'), 'w', encoding='utf8') as f:
        json.dump(json_vocab, f, indent=4)
    with open(os.path.join(out_dir, 'merge.txt'), 'w', encoding='utf8') as f:
        for p1, p2 in merges:
